    ...
    return (
      ...
      >
        ...
        {Array.from({ length: shotCount }).map((_, i) => {
          ...
          return (
            <div key={i} className={`h-10 w-full bg-gray-100 ${isSeparatorBorder ? 'border-b-2 border-black' : 'border-b border-gray-300'}`}></div>
          );
        })}
    ...
    return (
      ...
      >
        ...
        {Array.from({ length: shotCount }).map((_, i) => {
          ...
          return (
            <div key={index} className={`h-10 w-full bg-blue-50 flex items-center justify-center text-xl font-bold border-b ${isSeparator ? 'border-b-2 border-black' : 'border-gray-300'} text-blue-500`}>
      ...
      {Array.from({ length: shotCount }).map((_, i) => {
        ...
        return (
          <button 
            ...
            className={`h-10 w-full bg-white flex items-center justify-center text-xl font-bold border-b ${isSeparator ? 'border-b-2 border-black' : 'border-gray-300'} ${getMarkColorClass(mark)}`}
          >
            {mark || "　"}
          </button>
        );
  ...
  return (
    <div className="flex flex-col h-full bg-white absolute inset-0 z-20">
      ...
      <div className="flex justify-between items-center p-2 bg-gray-100 border-b">
        <button onClick={onBack} className="text-blue-500 px-2 py-1">戻る</button>
        <div className="flex items-center gap-4">
          <button disabled={currentIndex === sortedSessions.length - 1} onClick={() => switchSession(1)} className="p-2 disabled:opacity-30">
            <ChevronLeft size={24} />
          </button>
          <h2 className="font-bold">{dateStr}</h2>
          <button disabled={currentIndex === 0} onClick={() => switchSession(-1)} className="p-2 disabled:opacity-30">
      ...
      {!model.isAdminMode && activeSession.note && (
        <div className="p-3 bg-orange-50 text-sm border-b">
          <span className="font-bold">📝 メモ:</span> {activeSession.note}
        </div>
      )}

      {model.isAdminMode && (
        <div className="p-3 bg-orange-50 border-b space-y-2 text-sm">
          <div className="flex justify-between items-center">
            <span className="font-bold">矢数:</span>
            <select 
              value={activeSession.shotCount} 
              onChange={e => model.updateSessionShotCount(activeSession.id, parseInt(e.target.value))}
              className="bg-white border rounded px-2 py-1"
            >
              {[2,4,8,12,16,20].map(n => <option key={n} value={n}>{n}本</option>)}
            </select>
          </div>
          <input 
            type="text" 
      ...
      <div className="flex-1 overflow-auto p-4">
        <div className="flex flex-row-reverse w-max min-w-full justify-start">
          ...
          {/* Header Column */}
          <div className="flex flex-col border-l border-black w-8 flex-shrink-0">
            <div className="h-10 w-8 flex items-center justify-center bg-gray-200 border-b border-black text-xs font-bold">計</div>
            {Array.from({ length: activeSession.shotCount }).map((_, i) => {
              ...
              return (
                <div key={shotNum} className={`h-10 w-8 flex items-center justify-center bg-gray-100 border-b ${isSeparator ? 'border-b-2 border-black' : 'border-gray-300'} text-xs`}>
                  {shotNum}
                </div>
              );
            })}
            <div className="h-20 w-8 flex items-center justify-center bg-gray-200 border-b border-black text-xs">名</div>
          </div>
      ...
      {model.isAdminMode && (
        <div className="bg-white border-t p-4 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
          <div className="flex justify-center gap-4 mb-4">
            <button onClick={() => model.addSeparatorToSession(activeSession.id)} className="flex items-center gap-1 text-orange-500 bg-orange-50 px-3 py-2 rounded">
              <SplitSquareHorizontal size={18} /> 間隔
            </button>
            <button onClick={() => model.addTotalToSession(activeSession.id)} className="flex items-center gap-1 text-blue-500 bg-blue-50 px-3 py-2 rounded">
              <Sigma size={18} /> 計追加
            </button>
            <button onClick={() => model.addArcherToSession(activeSession.id)} className="flex items-center gap-1 text-white bg-blue-500 px-4 py-2 rounded font-bold">
              <UserPlus size={18} /> 射手
            </button>
          </div>
          <button 
            onClick={() => setShowDeleteAlert(true)} 
            className="w-full flex items-center justify-center gap-2 bg-red-500 text-white py-3 rounded font-bold"
          >
            <Trash2 size={18} /> この記録を削除
          </button>
        </div>
      )}

      {showDeleteAlert && (
        <div className="fixed inset-0 bg-transparent flex items-center justify-center z-50" onClick={() => setShowDeleteAlert(false)}>
          <div className="bg-white p-4 rounded-lg w-80 shadow-2xl border border-gray-200" onClick={e => e.stopPropagation()}>
            <h3 className="font-bold mb-2">記録の削除</h3>
            <p className="text-sm text-gray-600 mb-4">この記録を完全に削除しますか？この操作は取り消せません。</p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowDeleteAlert(false)} className="px-3 py-1 text-gray-600">キャンセル</button>
              <button 
                onClick={() => {
                  onBack();
      ...
      {showGuestAlert && (
        <div className="fixed inset-0 bg-transparent flex items-center justify-center z-50" onClick={() => setShowGuestAlert(false)}>
          <div className="bg-white p-4 rounded-lg w-64 shadow-2xl border border-gray-200" onClick={e => e.stopPropagation()}>
            ...
            />
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowGuestAlert(false)} className="px-3 py-1 text-gray-600">キャンセル</button>
              <button 
                onClick={() => {
                  if (editingArcherId) {
